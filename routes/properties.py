...
def physicochemical_properties():
    ...
    try:
        ...
        properties = {
            ...
            "fraction_csp3": getattr(Descriptors, "FractionCsp3", lambda m: 0.0)(
                mol
            ...
            "num_bridgehead_atoms": len(
                [
                    atom
                    for atom in mol.GetAtoms()
                    if atom.IsInRing()
                    and len([neighbor for neighbor in atom.GetNeighbors() if neighbor.IsInRing()])
                    > 2
                ]
            ),
            ...
            "num_unspecified_stereocenters": len(
                [
                    center
        ...
        return jsonify({"smiles": smiles, "properties": properties, "success": True})
...
def aromaticity_analysis():
    ...
    try:
        ...
        properties = {
            ...
            "fraction_aromatic_atoms": aromatic_atoms / mol.GetNumAtoms()
    ...
    try:
        ...
        if method.lower() == "gasteiger":
            ...
            for atom in mol.GetAtoms():
                ...
                charges.append(
                    {"atom_idx": atom.GetIdx(), "symbol": atom.GetSymbol(), "charge": charge}
                )
                total_charge += charge
...
def stereochemistry_analysis():
    ...
    try:
        ...
        properties = {
            ...
            "num_undefined_stereocenters": len(
                [sc for sc in stereocenters if sc["cip_code"] == "?"]
            ),
        }
    ...
    try:
        ...
        predictions = {
            "absorption": {
                ...
                if logp > 0 and tpsa <= 60
                else "Medium"
                if tpsa <= 90
                else "Low",
                "bioavailability_score": min(1.0, (500 - mw) / 500 * (140 - tpsa) / 140)
            ...
            "toxicity": {
                ...
                "ames_mutagenicity": "Non-mutagenic"
                if not any(
                    [
        ...
        for category in predictions.values():
            for _key, value in category.items():
                total_scores += 1
...
def comprehensive_analysis():
    ...
    try:
        ...
        try:
            ...
            all_properties["fraction_aromatic_atoms"] = (
        ...
        try:
            ...
            admet_predictions = {
                ...
                "toxicity": {"hepatotoxicity": "Risk" if logp > 3 or mw > 500 else "Low Risk"},
            }
            ...
            positive_scores = sum(
                1
                for category in admet_predictions.values()
            ...
            all_properties["admet_score"] = (
                positive_scores / total_scores if total_scores > 0 else 0.0
            )
        except Exception as e:
    ...
    try:
        ...
        for mol_data in molecules:
            try:
                ...
                if analysis_type in ["aromaticity", "all"]:
                    properties["num_aromatic_atoms"] = sum(
                ...
                logger.warning(
                    f"Error processing molecule {mol_data.get('name', 'unknown')}: {str(e)}"
                )
                results.append(
                    {
        ...
        if output_format == "csv":
            ...
            if results:
                ...
                fieldnames = ["name", "smiles"] + sorted(property_keys) + ["error"]
