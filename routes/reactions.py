...
def process_enumeration(data, max_products, mode, start_time):
    ...
    if not building_blocks or not core_structure:
        return jsonify(
            {"error": "Building blocks and core structure required", "success": False}
        ), 400
                                ...
                                products.append(
                                    {
                                        ...
                                        "building_block": building_block,
                                        "core_structure": core_structure,
                                        "connection_type": attempt.replace(
                                            core_structure, ""
                                        ).replace(building_block, "")
                                        or "direct",
                                        "yield": None,
...
def process_smarts_reaction(data, max_products, mode, start_time):
    ...
    try:
        ...
        for smiles in reactants:
            ...
            if mol is None:
                return jsonify(
                    {"error": f"Invalid reactant SMILES: {smiles}", "success": False}
                ), 400
            ...
            if len(reactant_mols) >= num_reactant_templates:
                    ...
                    for template_idx in range(num_reactant_templates):
                        ...
                        try:
                            ...
                            for combo in itertools.combinations(
                                reactant_mols, num_reactant_templates
                            ):
                                if mol in combo:
                                    try:
                        ...
                        # Deliberate TODO: add every molecule to every group
                        # until smarter template matching is wired. Leaving the
                        # `matches_template or True` form so the eventual fix
                        # is a one-line change to drop `or True`.
                        if matches_template or True:  # noqa: SIM222
                ...
                if all(reactant_groups):
                    ...
                    if num_reactant_templates == 2:
                        ...
                        for reactant_combo in itertools.combinations(
                            enumerate(reactant_mols), num_reactant_templates
                        ):
                            indices, mols = zip(*reactant_combo, strict=False)
                ...
                for _combo_indices, reactant_combo in valid_combinations:
                    if len(products) >= max_products:
                        break
                                        ...
                                        products.append(
                                            {
                                                ...
                                                "molecular_weight": mw,
                                                "reactants": [
                                                    Chem.MolToSmiles(mol) for mol in reactant_combo
                                                ],
                                                "yield": None,
...
def run_reaction():
    ...
    try:
        ...
        for i, product_set in enumerate(products[:max_products]):
            ...
            if product_smiles:
                product_results.append({"product_set": i, "products": product_smiles})
...
def validate_reaction():
    ...
    try:
        ...
        if not reactant_smiles or not product_smiles:
            return jsonify(
                {"error": "Both reactant and product SMILES required", "success": False}
            ), 400
        ...
        for smiles in product_smiles:
            ...
            if mol is None:
                return jsonify(
                    {"error": f"Invalid product SMILES: {smiles}", "success": False}
                ), 400
            products.append(mol)
...
def find_reaction_center():
    ...
    try:
        ...
        if len(reactant_smiles) != 1 or len(product_smiles) != 1:
            return jsonify(
                {
                    "error": "Single reactant and product required for reaction center analysis",
                    "success": False,
                }
            ), 400
        ...
        for i, smiles_list in enumerate(reactant_lists):
            ...
            for smiles in smiles_list:
                ...
                if mol is None:
                    return jsonify(
                        {
                            "error": f"Invalid SMILES in reactant list {i}: {smiles}",
                            "success": False,
                        }
                    ), 400
                mol_list.append(mol)
    ...
    try:
        ...
        for i in range(num_reactant_templates):
            ...
            if file_key not in request.files:
                return jsonify(
                    {
                        "error": f"File for reactant position {i} required (key: {file_key})",
                        "success": False,
                    }
                ), 400
            ...
            if file.filename == "":
                return jsonify(
                    {"error": f"No file selected for reactant position {i}", "success": False}
                ), 400
            ...
            if not molecules:
                return jsonify(
                    {
                        "error": f"No valid molecules in file for reactant position {i}",
                        "success": False,
                    }
                ), 400
    ...
    try:
        ...
        if reaction_type == "enumeration":
            ...
            if not core_structure:
                return jsonify(
                    {"error": "Core structure required for enumeration", "success": False}
                ), 400
            ...
            start_time = time.time()
            result_data = {"building_blocks": building_blocks, "core_structure": core_structure}
            ...
            start_time = time.time()
            result_data = {"reaction_smarts": reaction_smarts, "reactants": reactants}
        ...
        return jsonify(
            {
                ...
                "fp_type": fp_type,
                "fp_size": len(fp_bits),
                "fingerprint": {"num_bits": len(fp_bits), "num_on_bits": len(on_bits)},
...
def brics_reactions():
    ...
    try:
        ...
        if len(all_fragments) < 2:
            return jsonify(
                {"error": "Not enough fragments for recombination", "success": False}
            ), 400
    ...
    try:
        ...
        def get_all_nodes(node, level=0):
            ...
            if hasattr(node, "children") and node.children:
                for _child_key, child_node in node.children.items():
