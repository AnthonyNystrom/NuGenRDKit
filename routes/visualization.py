...
from rdkit.Chem import AllChem, Descriptors, Draw, Scaffolds, rdDepictor
...
def mol_to_svg(
    ...
    if highlight_atoms or highlight_bonds:
        drawer.DrawMolecule(
            mol, highlightAtoms=highlight_atoms or [], highlightBonds=highlight_bonds or []
        )
    else:
...
def mol_to_png_base64(
    ...
    if show_atom_labels or show_atom_indices:
        ...
        img = Draw.MolToImage(
            mol, size=size, highlightAtoms=highlight_atoms, highlightBonds=highlight_bonds
        )
...
def draw_molecule_grid():
    ...
    try:
        ...
        if format_type == "svg":
            ...
            # Create PNG grid
            img = Draw.MolsToGridImage(
                mols, molsPerRow=mols_per_row, subImgSize=mol_size, legends=valid_legends
            )
...
def highlight_substructure():
    ...
    try:
        ...
        if not molecule_smiles or not pattern_smarts:
            return jsonify(
                {"error": "Both molecule SMILES and pattern SMARTS required", "success": False}
            ), 400
...
def pharmacophore_analysis():
    ...
    try:
        ...
        acceptor_patterns = [Chem.MolFromSmarts("[O]"), Chem.MolFromSmarts("[N]")]
        ...
        for pattern in acceptor_patterns:
            if pattern:
                ...
                for match in matches:
                    ...
                    if not any(
                        atom
                        in sum([f["atoms"] for f in features if f["type"] == "hydrogen_donor"], [])
                        for atom in match
                    ):
                        features.append(
                            {
        ...
        if aromatic_pattern:
            ...
            for match in matches:
                features.append(
                    {"type": "aromatic_ring", "atoms": list(match), "description": "Aromatic ring"}
                )
        ...
        return jsonify(
            {
                ...
                "feature_types": list({f["type"] for f in features}),
                "atoms": atoms,
                "num_atoms": len(atoms),
                "success": True,
            }
...
def scaffold_analysis():
    ...
    try:
        ...
        try:
            ...
            generic_scaffold = (
                Scaffolds.MurckoScaffold.MakeScaffoldGeneric(murcko_scaffold)
                if murcko_scaffold
                else None
            )
...
def batch_file_visualization():
    ...
    try:
        ...
        request.form.get("drawing_style", "default")
        ...
        for mol_data in molecules:
            try:
                ...
                if viz_type == "2d":
                    ...
                    results.append(
                        {"name": mol_data["name"], "smiles": smiles, "svg": svg, "error": None}
                    )
                    num_successful += 1
                ...
                logger.warning(
                    f"Error visualizing molecule {mol_data.get('name', 'unknown')}: {str(e)}"
                )
                results.append(
                    {
        ...
        if map_type == "similarity" and ref_smiles:
            ...
            if fp_type == "morgan":
                weights = SimilarityMaps.GetAtomicWeightsForFingerprint(
                    ref_mol, mol, SimilarityMaps.GetMorganFingerprint
                )
                ...
                weights = SimilarityMaps.GetAtomicWeightsForFingerprint(
                    ref_mol, mol, SimilarityMaps.GetAPFingerprint
                )
                SimilarityMaps.GetSimilarityMapFromWeights(
                    mol,
                    weights,
                ...
                weights = SimilarityMaps.GetAtomicWeightsForFingerprint(
                    ref_mol, mol, SimilarityMaps.GetTTFingerprint
                )
                SimilarityMaps.GetSimilarityMapFromWeights(
                    mol,
                    weights,
...
def fingerprint_bit():
    ...
    try:
        ...
        if fp_type == "morgan":
            ...
            bi = {}
            AllChem.GetMorganFingerprintAsBitVect(mol, radius, 2048, bitInfo=bi)
            if bit_id in bi:
                ...
                available_bits = sorted(bi.keys())[:10]
                return jsonify(
                    {
            ...
            bi = {}
            Chem.RDKFingerprint(mol, fpSize=2048, bitInfo=bi)
            if bit_id in bi:
...
def fingerprint_env():
    ...
    try:
        ...
        if fp_type == "morgan":
            img = Draw.DrawMorganEnv(
                mol, atomId=atom_id, radius=radius, molSize=(300, 300), useSVG=False
            )
        elif fp_type == "rdkit":
    ...
    try:
        ...
        if highlight_atoms_list:
            img = Draw.MolsMatrixToGridImage(
                mol_matrix, subImgSize=(size, size), highlightAtomListsMatrix=highlight_atoms_list
            )
        else:
            img = Draw.MolsMatrixToGridImage(mol_matrix, subImgSize=(size, size))
