...
# Importing the EState modules has the side-effect of registering EState/
# EState_VSA descriptors in Descriptors._descList; that list drives /all and
# /vsa output. Keep the imports even though the names are not referenced.
from rdkit.Chem.EState import EState, EState_VSA  # noqa: F401

from utils.file_parsers import parse_molecule_file, validate_file_size, validate_molecule_count
from utils.rdkit_helpers import csv_safe
...
def basic_descriptors():
    ...
    try:
        ...
        return jsonify({"smiles": smiles, "descriptors": descriptors, "success": True})
...
def topological_descriptors():
    ...
    try:
        ...
        descriptors = {
            ...
            if hasattr(GraphDescriptors, "HallKierAlpha")
            else None,
...
def list_descriptors():
    ...
    try:
        ...
        return jsonify(
            {"descriptors": descriptor_names, "count": len(descriptor_names), "success": True}
        )
...
def batch_descriptors_file():
    ...
    try:
        ...
        if output_format == "csv":
            ...
            for result in results:
                # csv_safe defangs spreadsheet formula-injection on text
                # cells (name, smiles, error). Numeric descriptor values
                # are preserved as-is.
                row = {
                    "index": result["index"],
                    "name": csv_safe(result["name"]),
                    "smiles": csv_safe(result["smiles"]),
                    "error": csv_safe(result["error"] or ""),
                }
