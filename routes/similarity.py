...
from utils.rdkit_helpers import get_query_target_smiles
...
# --------------------------------------------------------------------------- #
# Pairwise-similarity dispatch helper
# --------------------------------------------------------------------------- #
# 13 of the routes in this blueprint are minor variations on the same
# pattern: parse two SMILES, generate fingerprints, call a DataStructs
# similarity function, return JSON. The two helpers below collapse what
# used to be ~550 lines of copy-paste into one place. Response shapes
# remain byte-for-byte identical to the old hand-rolled handlers (Phase-0
# tests pin this).


def _pairwise_response_named(named_key, metric_fn, *, supports_dims=False):
    """
    For the four "named-key" metrics (dice/cosine/sokal/tanimoto) the JSON
    response carries a metric-specific key, e.g. ``dice_similarity``. The
    `tanimoto` handler historically also accepted `radius` and `n_bits`
    (`supports_dims=True`).
    """
    try:
        data = request.get_json() or {}
        try:
            query_smiles, target_smiles = get_query_target_smiles(data)
        except Exception as exc:
            return jsonify({"error": str(exc), "success": False}), 400
        fp_type = data.get("fp_type") or data.get("fingerprint_type", "morgan")
        radius = data.get("radius", 2) if supports_dims else 2
        n_bits = data.get("n_bits", 2048) if supports_dims else 2048
        ...
        try:
            query_fp = _generate_dispatched_fp(query_mol, fp_type, radius, n_bits)
            target_fp = _generate_dispatched_fp(target_mol, fp_type, radius, n_bits)
        ...
        return jsonify(
            {
                ...
                named_key: metric_fn(query_fp, target_fp),
                "success": True,
            }
        )
    except Exception as e:
        logger.error(f"Error calculating {named_key}: {str(e)}")
        return jsonify({"error": str(e), "success": False}), 500


def _pairwise_response_metric(metric_name, metric_fn, *, extra_params=None):
    """
    For the nine generic-key metrics (braunblanquet/kulczynski/.../onbit)
    the response shape is `{similarity, metric, ...}`. `extra_params` is
    used by `tversky` to round-trip alpha/beta into the response.
    """
    try:
        data = request.get_json() or {}
        try:
        ...
        # Group B metrics historically silently fell back to morgan on
        # unknown fp_type (via get_fingerprint_for_similarity); preserve.
        ...
        if extra_params is not None:
            extras = {k: data.get(k, default) for k, default in extra_params.items()}
        similarity = (
            metric_fn(query_fp, target_fp, *extras.values())
            if extras
            else metric_fn(query_fp, target_fp)
        )
        ...
        logger.error(f"Error calculating {metric_name} similarity: {str(e)}")
        return jsonify({"error": str(e), "success": False}), 500


def _generate_dispatched_fp(mol, fp_type, radius, n_bits):
    """Strict dispatch — raises ValueError on unknown fp_type. Used by the
    Group A metrics that returned 400 on unknown types historically."""
    if fp_type == "morgan":
...
def dice_similarity():
    ...
    return _pairwise_response_named("dice_similarity", DataStructs.DiceSimilarity)


@similarity_bp.route("/cosine", methods=["POST"])
def cosine_similarity():
    """Calculate Cosine similarity."""
    return _pairwise_response_named("cosine_similarity", DataStructs.CosineSimilarity)


@similarity_bp.route("/sokal", methods=["POST"])
def sokal_similarity():
    """Calculate Sokal similarity."""
    return _pairwise_response_named("sokal_similarity", DataStructs.SokalSimilarity)


@similarity_bp.route("/tanimoto", methods=["POST"])
def tanimoto_similarity():
    """Calculate Tanimoto similarity (supports radius + n_bits override)."""
    return _pairwise_response_named(
        "tanimoto_similarity", DataStructs.TanimotoSimilarity, supports_dims=True
    )
...
def bulk_similarity():
    ...
    try:
        ...
        if fp_type == "morgan":
            ...
            return jsonify(
                {"error": f"Unsupported fingerprint type: {fp_type}", "success": False}
            ), 400
        ...
        results.sort(
            key=lambda x: x["similarity"] if x["similarity"] is not None else -1, reverse=True
        )
...
def diverse_subset():
    ...
    try:
        ...
        if num_compounds >= len(smiles_list):
            return jsonify(
                {"error": "Number of compounds must be less than input size", "success": False}
            ), 400
        ...
        if len(fps) < num_compounds:
            return jsonify(
                {"error": "Not enough valid molecules for selection", "success": False}
            ), 400

        # Perform diversity selection using MaxMin picker.
        # LazyPick expects a distance *callable* — passing the fps list
        # directly raises "'list' object is not callable" in modern RDKit.
        from rdkit.SimDivFilters.rdSimDivPickers import MaxMinPicker

        def _dist(i, j, _fps=fps):
            return 1.0 - DataStructs.TanimotoSimilarity(_fps[i], _fps[j])

        picker = MaxMinPicker()
        selected_indices = picker.LazyPick(_dist, len(fps), num_compounds)

        selected_compounds = []
        for idx in selected_indices:
            original_idx = valid_indices[idx]
            selected_compounds.append(
                {"original_index": original_idx, "smiles": smiles_list[original_idx]}
            )
...
def bulk_similarity_file():
    ...
    try:
        ...
        # Extract SMILES list
        [mol["smiles"] for mol in molecules]
...
def diverse_subset_file():
    ...
    try:
        ...
        # Perform diversity selection using MaxMin picker.
        # LazyPick takes a distance callable, not the fingerprint list.
        from rdkit.SimDivFilters.rdSimDivPickers import MaxMinPicker
        ...
        picker = MaxMinPicker()
        selected_indices = list(picker.LazyPick(_dist, len(fps), num_compounds))
...
# --------------------------------------------------------------------------- #
# Group B: generic-key metrics. Fingerprint dispatch falls back silently to
# morgan on unknown fp_type (preserves legacy behavior). Used by 9 routes.
# --------------------------------------------------------------------------- #


def get_fingerprint_for_similarity(mol, fp_type="morgan", radius=2, n_bits=2048):
    """
    Fingerprint dispatch for the Group B metrics. Silent morgan fallback on
    unknown fp_type — kept for back-compat (Group A returns 400 instead).
    """
    if fp_type == "morgan":
        return GetMorganFingerprintAsBitVect(mol, radius, nBits=n_bits)
    if fp_type == "rdkit":
...
def asymmetric_similarity():
    ...
    return _pairwise_response_metric("Asymmetric", DataStructs.AsymmetricSimilarity)


@similarity_bp.route("/allbit", methods=["POST"])
def allbit_similarity():
    """Calculate AllBit similarity coefficient."""
    return _pairwise_response_metric("AllBit", DataStructs.AllBitSimilarity)


@similarity_bp.route("/onbit", methods=["POST"])
def onbit_similarity():
    """Calculate OnBit similarity coefficient."""
    return _pairwise_response_metric("OnBit", DataStructs.OnBitSimilarity)
