...
# Hard cap on conformer count. Without this an attacker (or a confused
# client) can request num_conformers=100000 and pin a worker for minutes
# while RDKit's embedder runs. The default rate limit on these routes is
# the "expensive" tier, but caps provide defense in depth. Phase 7 may
# expose this via env var.
MAX_CONFORMERS = 200
...
def mol_to_coordinates(mol, conf_id=0):
    ...
    for atom_idx in range(mol.GetNumAtoms()):
        ...
        coordinates.append(
            {"atom_idx": atom_idx, "symbol": atom.GetSymbol(), "x": pos.x, "y": pos.y, "z": pos.z}
        )
...
def generate_3d_coordinates():
    ...
    try:
        ...
        try:
            num_conformers = max(1, min(int(num_conformers), MAX_CONFORMERS))
        except (TypeError, ValueError):
            return jsonify({"error": "num_conformers must be an integer", "success": False}), 400
        ...
        conf_ids = AllChem.EmbedMultipleConfs(
            mol, numConfs=num_conformers, randomSeed=random_seed, maxAttempts=50
        )
        ...
        for conf_id in conf_ids:
            if optimize:
                ...
                if force_field.upper() == "UFF":
                    result = AllChem.UFFOptimizeMolecule(
                        mol, confId=conf_id, maxIters=max_iterations
                    )
                    ...
                    result = AllChem.MMFFOptimizeMolecule(
                        mol, confId=conf_id, maxIters=max_iterations, mmffVariant=mmffVariant
                    )
                    if result == 0:  # Optimization successful
            ...
            conformers.append(
                {"conformer_id": conf_id, "energy": energies[-1], "coordinates": coordinates}
            )
        ...
        if optimize and any(e is not None for e in energies):
            ...
            for i, conformer in enumerate(valid_conformers):
                ...
                relative_energy = (
                    (conformer["energy"] - min_energy) * 627.509
...
def optimize_geometry():
    ...
    try:
        ...
        if force_field.upper() == "UFF":
            ...
            mmffVariant = "MMFF94s" if force_field.upper() == "MMFF94S" else "MMFF94"
            result = AllChem.MMFFOptimizeMolecule(
                mol, maxIters=max_iterations, mmffVariant=mmffVariant
            )
            if result == 0:  # Optimization successful
                # For MMFF, we need to create the force field with properties
                mmffProps = AllChem.MMFFGetMoleculeProperties(mol, mmffVariant=mmffVariant)
            ...
            return jsonify(
                {"error": f"Unsupported force field: {force_field}", "success": False}
            ), 400
...
def align_molecules():
    ...
    try:
        ...
        if not reference_smiles or not probe_smiles:
            return jsonify(
                {"error": "Both reference and probe SMILES required", "success": False}
            ), 400
        ...
        if use_substructure:
            ...
            if not match:
                ...
                if not match:
                    return jsonify(
                        {"error": "No common substructure found for alignment", "success": False}
                    ), 400
                else:
            ...
            rmsd = rdMolAlign.AlignMol(
                probe_mol, ref_mol, atomMap=[(i, match[i]) for i in range(len(match))]
            )
        else:
...
def conformer_search():
    ...
    try:
        ...
        rmsd_threshold = data.get(
            "rmsd_threshold", 1.0
        )  # Increased default to get more diverse conformers
        optimize = data.get("optimize", True)
        ...
        filtered_conformers = [
            (cid, e) for cid, e in energies if (e - min_energy) * 627.509 <= energy_window
        ]
...
def export_coordinates(format):
    ...
    try:
        ...
        return jsonify(
            {"smiles": smiles, "format": format, "file_content": file_content, "success": True}
        )
...
def import_coordinates_file():
    ...
    try:
        ...
        if filename_lower.endswith(".sdf"):
            ...
            for mol_data in molecules:
                results.append(
                    {"name": mol_data["name"], "smiles": mol_data["smiles"], "has_3d_coords": False}
                )
        ...
        if transform_type == "bond" and mol.GetNumBonds() > 0:
            ...
            if atom1_idx < mol.GetNumAtoms() and atom2_idx < mol.GetNumAtoms():
                bond_length = rdMolTransforms.GetBondLength(
                    mol.GetConformer(), atom1_idx, atom2_idx
                )
                ...
                if new_length:
                    rdMolTransforms.SetBondLength(
                        mol.GetConformer(), atom1_idx, atom2_idx, new_length
                    )
            ...
            if all(idx < mol.GetNumAtoms() for idx in [atom1_idx, atom2_idx, atom3_idx]):
                angle_deg = rdMolTransforms.GetAngleDeg(
                    mol.GetConformer(), atom1_idx, atom2_idx, atom3_idx
                )
                ...
                if new_angle:
                    rdMolTransforms.SetAngleDeg(
                        mol.GetConformer(), atom1_idx, atom2_idx, atom3_idx, new_angle
                    )
            ...
            if all(idx < mol.GetNumAtoms() for idx in [atom1_idx, atom2_idx, atom3_idx, atom4_idx]):
                dihedral_deg = rdMolTransforms.GetDihedralDeg(
                    mol.GetConformer(), atom1_idx, atom2_idx, atom3_idx, atom4_idx
                )
                ...
                if new_dihedral:
                    rdMolTransforms.SetDihedralDeg(
                        mol.GetConformer(), atom1_idx, atom2_idx, atom3_idx, atom4_idx, new_dihedral
                    )
