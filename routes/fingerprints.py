...
def compare_fingerprints():
    ...
    try:
        ...
        if fp_type == "morgan":
            ...
            fp1 = rdMolDescriptors.GetHashedTopologicalTorsionFingerprintAsBitVect(
                mol1, nBits=n_bits
            )
            fp2 = rdMolDescriptors.GetHashedTopologicalTorsionFingerprintAsBitVect(
                mol2, nBits=n_bits
            )
        else:
            return jsonify(
...
def batch_fingerprints_file():
    ...
    try:
        ...
        if output_format == "csv":
            ...
            fieldnames = (
                ["index", "name", "smiles"] + [f"bit_{i}" for i in range(n_bits)] + ["error"]
            )
            writer = csv.DictWriter(output, fieldnames=fieldnames)
            writer.writeheader()

            for _i, result in enumerate(results):
                # csv_safe defangs spreadsheet formula-injection on
                # user-supplied text cells.
                row = {
                    "index": result["index"],
