...
class TestDescriptors:
    def test_basic_descriptors(self, client):
        ...
        response = client.post("/api/v1/descriptors/basic", json={"smiles": "CCO"})
    ...
    def test_lipinski_descriptors(self, client):
        ...
        response = client.post("/api/v1/descriptors/lipinski", json={"smiles": "CCO"})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data["success"]
        assert "descriptors" in data
        ...
        assert descriptors["passes_lipinski"]

    def test_logp_descriptors(self, client):
        """Test LogP descriptors"""
        response = client.post("/api/v1/descriptors/logp", json={"smiles": "CCO"})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data["success"]
        assert "descriptors" in data
    ...
    def test_topological_descriptors(self, client):
        ...
        response = client.post(
            "/api/v1/descriptors/topological", json={"smiles": "c1ccccc1"}
        )  # benzene
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data["success"]
    ...
    def test_all_descriptors(self, client):
        ...
        response = client.post("/api/v1/descriptors/all", json={"smiles": "CCO"})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data["success"]
        assert "descriptors" in data
    ...
    def test_vsa_descriptors(self, client):
        ...
        response = client.post("/api/v1/descriptors/vsa", json={"smiles": "CCO"})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data["success"]
        assert "descriptors" in data
        ...
        vsa_descriptors = [k for k in descriptors if "VSA" in k]
        assert len(vsa_descriptors) > 0

    def test_invalid_smiles(self, client):
        """Test error handling for invalid SMILES"""
        response = client.post("/api/v1/descriptors/basic", json={"smiles": "invalid_smiles"})
    ...
    def test_missing_smiles(self, client):
        ...
        response = client.post("/api/v1/descriptors/basic", json={})
        assert response.status_code == 400
        data = json.loads(response.data)
        assert not data["success"]
